...
parser.add_argument("--save_dir", default='./checkpoint_20220426', type=str,
...
parser.add_argument("--epochs", default=500, type=int, help="Total number of training epochs to perform.")
parser.add_argument("--warmup_proportion", default=0.25, type=float,
...
def read_custom_data(data_path):
    with open(data_path, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            #print(line.strip('\n').split('\t'))
            querys, labels = line.strip('\n').split(',')
            # print("{},{},{}".format(querys, titles, labels))
...
def do_train():
    ...
    # 改为自定义数据
    train_ds = load_dataset(read_custom_data, data_path='./train.csv', lazy=False)
    dev_ds = load_dataset(read_custom_data, data_path='./test.csv', lazy=False)
    ...
    for epoch in range(0, args.epochs + 1):
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            #lables_one_hot = paddle.nn.functional.one_hot(labels, args.cls_size)
            #lables_one_hot = paddle.squeeze(lables_one_hot, axis=1)
            #print(probs)
